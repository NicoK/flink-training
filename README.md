...
#### Run an Exercise from CLI

```bash
./gradlew :rides-and-fares:run
```

#### Run an Exercise's (java) solution from CLI

```bash
./gradlew :rides-and-fares:runSolution
```

#### Force a Re-run of all Tests
...
[`build.gradle`](build.gradle) file and adapt accordingly:
...
> the training requirements low for participants.
